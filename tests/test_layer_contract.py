...
class LayerContractTests(unittest.TestCase):
    ...
    def test_runtime_builder_has_strict_proofs(self) -> None:
        ...
        self.assertNotIn("rpcrt4", (ROOT / "compat" / "pwsh-policy.reg").read_text(encoding="utf-8"))
    ...
    def test_runtime_builder_requires_behavioral_proofs_and_manifest(self) -> None:
        ...
        self.assertIn("values = [int(value) for value in sys.argv[2:32]]", source)
        self.assertIn("logs_path = Path(sys.argv[32])", source)
        self.assertIn("markers = [Path(value) for value in sys.argv[33:]]", source)
